...
- 🎨 **Color-coded difficulty levels**: Green (◆) for beginner, Blue (◈) for intermediate, Pink (◉) for advanced
- 📊 **Progress tracking**: Mark katas as complete with `Space` and track your overall progress
- 🔍 **Split view layout**: Kata list on the left, detailed info on the right (responsive to terminal size)
- ⚡ **Subtle ASCII art**: Clean header design that doesn't overwhelm
- ⌨️ **Keyboard navigation**:
  - `↑/↓` or `j/k` to navigate katas
  - `←/→` or `a/d` to switch difficulty filters
  - `Space` to mark kata as complete/incomplete
  - `Q` or `Esc` to exit

Your progress is automatically saved to `~/.ts-mastery-progress.json`.
